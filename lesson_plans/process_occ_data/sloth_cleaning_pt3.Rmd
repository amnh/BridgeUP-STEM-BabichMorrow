...
**Try it yourself:** We can check how many occurrences were taken out by spatial thinning. Use `nrow()` to find how many rows (occurrences) were in your occurrences subsetted by polygon. Compare to the number of rows in `thinned_occs()`:
...
**Try it yourself:** Copy and paste the above code into the code chunk below. Use the `+` to add another layer onto your map: plot the points from `thinned_occs` on your map in a different color. You may need to zoom into a region of the map to see a difference.
...
We just spatially thinned our data to 10 km. This may or may not have been enough to remove the spatial bias in the data. Try the above process again, but increase the thinning distance to something higher than 10 km. Make sure to plot the thinned points on the map at the end -- which thinning distance results in data that looks less biased? Why?
