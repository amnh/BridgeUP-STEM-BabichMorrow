...
**Try it yourself:** Run just the `which()` portion of the above code. By nesting `which()` within the square brackets, we are basically doing the same thing we did earlier when we specified certain row numbers to keep, only this time we are letting `which()` do the work of picking out the rows to keep.
...
**Try it yourself:** Pick a different latitude or longitude (or both) range of occurrences to plot. You can use either `which()` or `subset()`. You can use `>`, `<`, `<=`, `>=`, `==`, or `!=` to specify values you do (or don't) want included. You can also use `&` to specify multiple conditions that must all be true. If you want to be really fancy, you could also use `|`, which means "or" in R. All of these symbols are *logical operators* in R.
