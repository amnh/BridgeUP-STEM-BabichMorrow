...
Our first step is to load in our occurrence data. Import the csv into R for the species you are working on:
...
To create the bounding box, we are going to use the `bbox()` function on the latitude and longitude columns of our species data.

**Try it yourself:** Complete the following code by subsetting your occurrence data just to include the longitude and latitude columns. Also, add a line of code to plot the bounding box.
...
For the environmental data, we are going to use the WorldClim bioclimatic variables you downloaded in `worldclim_inR.Rmd`. The raster stack containing these variables should be in your R environment as the object `env_stack`.

**Try it yourself:** Crop the environmental variables by the bounding box. Plot the resulting area to see what region the bounding box covers. Remember, cropping trims down the environmental raster to the extent of the bounding box.
...
**Try it yourself:** Mask the bounding box background extent shape from the cropped raster. Remember, masking takes the environmental raster and removes all regions that have an NA value in the bounding box (all cells outside the box).
...
**Try it yourself:** Now go through the same cropping and masking procedure as above to create an environmental raster for your study region based on the buffered bounding box:
...
The minimum convex polygon method draws the smallest polygon around the occurrence points where all angles of the polygon are less than 180°. To create the minimum convex polygon, we are going to create a function called `mcp()`.

**Try it yourself:** The `mcp()` function has already been written for you, but look over it to try to understand each line. Some of this code should look familiar to you from our lessons on Spatial objects in R.
...
**Try it yourself:** Create a minimum convex polygon called `bgExt` by providing it with your occurrence data (just the latitude and longitude columns!) and using the new `mcp()` function. Hint, you may need to subset out only the columns you need.
...
The third method of designating a study region is by creating a buffered region around each occurrence point. You can think of this as creating a bunch of circles of a given diameter (the buffer distance) around each occurrence point. To create this kind of background region, you can use the `gBuffer()` function directly on the occurrence points, instead of on a shape like the bounding box or MCP.
