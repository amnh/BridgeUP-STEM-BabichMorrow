...
Portions of the following tutorial are modified from Drew Kerkhoff's RPub https://rpubs.com/kerkhoffa/KenyonBasics and Nathaniel Phillips' *YaRrr! The Pirate's Guide to R* (https://bookdown.org/ndphillips/YaRrr/).

## Intro to RMarkdown

This file is an R Markdown file, which is similar to the Jupyter notebooks you've worked with. R Markdown files can save code, run code, and generate reports. They contain three basic types of content: the header at the top with title, author, etc.; code chunks surrounded by ` ``` `, and text (which can be formatted in different ways). We'll learn more about R Markdown later on, but for now, here are the basics:
...
** Try it yourself ** Knit the file as either a pdf or html file to see what the formatted product looks like.
...
**Try it yourself:** Change the first element in intern_names to your own name. Make sure to write the code to do this in the code chunk above so you can save it for future reference. Hint: you can reassign elements in a vector in the same way you would reassign a variable.

**Try it yourself:** Now that you have individually reassigned the first element in intern_names to your name, change the other names in the vector to match the names of the rest of the SlothSquad! There are (at least) two ways to do this -- see if you can figure out both.

Now let's add some numerical data using the distances from the icebreaker:

```{r}
destination_dists = c(42, 45, 110, 68, 54, 76)
```

Make sure you enter the distances to the destinations in the same order as the names in your intern_names vector. In order to keep track of the list of numbers you just entered, you can name the destination_dists vector using the intern_names vector you've already created:

```{r}
names(destination_dists) <- intern_names
```
...
Now let's try another histogram, this time using the 1000 pirate heights. What happens if we try basically the same thing we did for destination_dists?
...
Make sure to save your script with all the code you've run today! Once you save, commit those changes and push them to your fork.

##Recap

Stuff you've done in R!

* Become more familiar with the RStudio set-up
    + console, projects, scripts, environment, and help
* Integrated RStudio and GitHub to keep track of your edits
...
If you finish all of the above, keep working with the pirate dataset to learn about basic statistics and plotting in R. Save any new code you write in this file (or in anothe script) to keep track of what you did.

Calculate some descriptive statistics about the data: https://bookdown.org/ndphillips/YaRrr/descriptive-statistics.html

Make a scatterplot: first part of https://bookdown.org/ndphillips/YaRrr/plotting.html

Advanced scatterplot techniques: apply the techniques from https://www.statmethods.net/graphs/scatterplot.html to modify the scatterplot you made
