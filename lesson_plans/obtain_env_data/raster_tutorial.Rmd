...
The function `crop()` allows you to crop the raster to a given region. We can crop rasters in a couple of different ways. We can click on the plot of the raster itself to draw the region we want to crop by using the function `drawExtent()` -- when you run that line, click once on the upper left and once on the lower right to define a box.
...
The `merge()` function can make multiple rasters into a single new object as long as the original rasters have the same resolution and origin. We can merge the last two cropped rasters we created:
...
We can also access raster values at given xy-coordinates using the `extract()` function, which is what we will be doing with our sloth occurrence data!